...
- First 8-GPU launch attempt: Slurm job `6959201` started on `cbcb28` but was cancelled after `00:04:14` before trainer steps, checkpoints, or run cards were produced. Diagnosis from `nvidia-smi`: all 8 `torchrun` ranks loaded the rollout-format-gate model onto GPU0, leaving GPUs 1-7 idle. Root cause: the pre-trainer rollout gate used `model.to("cuda")` without respecting `LOCAL_RANK`, and nonzero ranks duplicated gate work.
- Fix before relaunch: `train_grpo.py` now sets the local CUDA device from `LOCAL_RANK`, runs rollout-format gate only on rank0, has nonzero ranks wait for the rank0 gate metrics, and restricts post-training artifact generation to rank0. This keeps the expensive trainer path aligned with the 8-GPU allocation.
- Second 8-GPU launch attempt: Slurm job `6959210` started correctly with rank0-only gate behavior but was cancelled after `00:06:51` before trainer steps. Diagnosis: the gate inherited the full training rollout settings (`sample_count=16`, `sample_rollout_generations=4`, `max_completion_length=2048`), making the preflight itself too expensive while GPUs 1-7 waited.
- Fix before relaunch: rollout-format gate now supports explicit lightweight overrides. The one-epoch config keeps the trainer rollout at `max_completion_length=2048`, `num_generations=4`, `temperature=0.8`, but uses a cheap gate of `4` prompts, `1` completion per prompt, and `max_completion_length=512`.
- Third 8-GPU launch attempt: Slurm job `6959216` completed the lightweight gate quickly but failed before trainer steps because `parse_failure_rate=0.75` exceeded the strict `0.5` threshold. The gate samples showed three hard first-in-file DAPO prompts did not reach a boxed final answer within the 512-token gate, while one prompt produced a boxed but wrong answer and received the `math_boxed_verl_v001` format credit `0.1`.
...
### 2026-06-03 Qwen2.5-Math-1.5B MATH500 vLLM Full 500x16 Eval

- Goal: run the full MATH500 validation protocol with `500` examples and `16` sampled completions per example, using vLLM batching so the result is comparable to the paper's "average Pass@1 over 16 samples" protocol.
- Code commit: `a67a563`, which adds the optional `vllm` inference backend to `math_dataset_eval.py`; this run used a clean server worktree at `/fs/nexus-scratch/qhe123/posttrain-lab-worktrees/a67a563-math500-vllm-full16`.
- Dataset/model: read-only MATH500 test parquet at `/fs/nexus-scratch/qhe123/datasets/verl_parquet/math500/test.parquet`; model `/fs/nexus-scratch/qhe123/models/Qwen2.5-Math-1.5B`.
- Paper-aligned eval settings: `500` MATH500 examples, `16` samples per example (`8000` completions), `prompt_template=paper_dapo`, chat template enabled, `max_new_tokens=2048`, `temperature=0.8`, `top_p=1.0`, `extra_eos_token=<|im_end|>`, `reward_version=math_boxed_verl_v001`, `allow_symbolic_equivalence=true`, `symbolic_equivalence_engine=sympy`.
- Nexus/runtime settings: Slurm job `6959941`, one RTX 6000 Ada, vLLM `0.11.0`, batch size `512`, `vllm_max_model_len=3072`, `VLLM_USE_FLASHINFER_SAMPLER=0` because FlashInfer sampler JIT required `nvcc` on this node. Wall time was `00:10:36`.
- Smoke before full: Slurm job `6959936`, `5` examples x `16` samples, completed in `00:01:03`; accuracy `0.65`, format success `0.9875`, parse failure `0.0125`, pass@16 `1.0`.
- Full metrics: completion-level full-credit accuracy `0.444375` (`3555/8000`), prompt-level pass@16 `0.856` (`428/500`), format success `0.933875`, parse failure `0.066125`, correctness given parse `0.47584`, truncation `0.10625`, average completion length `758.63` tokens.
- Reason counts: exact match `3390`, sympy equivalence `165`, format-correct wrong answer `3916`, empty boxed answer `207`, no boxed answer `285`, malformed boxed answer `36`, output too long `1`.
- Strict paper comparison: the paper's Figure 2 reports MATH500 for Qwen2.5-Math-1.5B as average Pass@1 over `16` samples; visually, the initial point is about `0.55`, RLOO ends around `0.68`, and JS ends around `0.70`. Our directly comparable number is completion-level accuracy `0.4444`, not pass@16. Therefore our local base-model eval remains roughly `10` percentage points below the paper's plotted initial MATH500 point and far below the trained RLOO/JS endpoints.
- Comparability caveats: the paper prompt example uses a system message, while our `paper_dapo` template appends the same boxed-answer instruction to the user problem; the paper's exact verifier/parser implementation is not provided in the PDF; this run used one RTX 6000 Ada instead of the paper's `4xGH200`, but compute hardware affects speed rather than the sampled metric.
- Interpretation: vLLM solved the throughput issue and produced the requested full 500x16 sample. The low `44.44%` completion-level accuracy is no longer explained by single-sample variance or transformers generation slowness. The remaining gap is likely due to prompt-template mismatch, tokenizer/chat-template differences, and verifier/parser differences, plus real model behavior: `10.6%` truncation and `6.6%` parse failures still remove many otherwise possible correct samples.
- Safety: no `data/raw` files, MATH500 prompts/answers, reward semantics, or train/validation/test splits were modified.
